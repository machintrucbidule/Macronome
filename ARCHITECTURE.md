...
```
                 ┌─────────────────────────────────────────────┐
   Browser ──────│  reverse proxy (deployment-agnostic)         │
   (React SPA)   │  · serves the static SPA build               │
                 │  · proxies /api/* → API                      │
                 │  · terminates TLS (or a frontal does)        │
                 └───────────────┬─────────────────────────────┘
                                 │ HTTP (trusted-proxy headers only)
                          ┌──────▼───────┐         ┌──────────────┐
                          │  API         │◄───────►│  PostgreSQL  │
                          │  Node + TS   │ Prisma  │  (1 volume)  │
                          │  Express 5   │         │  unaccent,   │
                          │  domain +    │         │  pg_trgm     │
                          │  services +  │         └──────────────┘
                          │  repositories│
...
The proxy is **not** a product decision: the app is a standard web service. A
default reverse proxy ships in the compose file so it runs out of the box; any
frontal (an opened port, a tunnel, a cloud load balancer) can replace it. The API
trusts forwarded client-IP headers **only from the configured proxy** so login
rate-limiting sees the real client regardless of frontal (see `security.md`).
...
| Packaging (prod) | **Docker Compose** (`proxy` · `api` · `postgres`) | Reproducible, deployment-agnostic, trivial standard `pg_dump` backups. |
...
is still *persisted* (written by the service on each day mutation, frozen via the
