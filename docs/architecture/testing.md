...
test('BMR Mifflin-St Jeor (neutral oracle)', () => {
  ...
  expect(bmr({ weightKg: 80, heightCm: 180, age: 40, sex: 'male' })).toBeCloseTo(1730, 1); // display precision: 1 dp
});

test('empirical burn/day = avgIntake + lostKg*7700/days (neutral oracle)', () => {
  // spec §4: 2000 + 0.5*7700/7
  expect(empiricalBurnPerDay({ avgIntake: 2000, weightStart: 80, weightEnd: 79.5, days: 7 })).toBe(
    2550,
  );
});
```
...
- It runs in **CI on every push** alongside lint/typecheck, and is part of the
  _done_ rule below: a change that edits the schema is not done until `check:schema`
  is green. When the two disagree, **the contract wins** — fix `schema.prisma`, never
  the contract (`CLAUDE.md` → _Fixed contracts_).

---

## 6. "Done" — the self-verification rule

For any change the agent makes, _done_ means all of:
...
integration on every push, and **e2e on every push to `main`** (plus on PRs) — pipeline
details are ops. The e2e step was PR-only until B-246; since this project pushes straight
to `main`, that meant it never ran and four specs rotted unnoticed.
