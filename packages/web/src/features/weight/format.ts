...
export const kg1 = (n: number): string => n.toFixed(1);
export const bmi1 = (n: number): string => n.toFixed(1);
export const kcal0 = (n: number): string => Math.round(n).toString();
export const mult2 = (n: number): string => `×${n.toFixed(2)}`;

/** Signed delta with a real minus sign: "+0.5", "−1.0", "0.0". */
export const signed1 = (n: number): string =>
  `${n > 0 ? '+' : n < 0 ? MINUS : ''}${Math.abs(n).toFixed(1)}`;
