...
interface CatalogViewProps {
  /** Shared with Mes aliments — switching mode keeps what the user typed. */
  q: string;
  onQ: (q: string) => void;
...
export function CatalogView(props: CatalogViewProps) {
  ...
  const params = buildRefListParams(filters.state, props.q, locale);

  const list = useFoodRefsList(params);
  const groups = useFoodRefGroups(locale);
  const refs = useMemo(() => list.data?.pages.flatMap((p) => p.data) ?? [], [list.data]);
