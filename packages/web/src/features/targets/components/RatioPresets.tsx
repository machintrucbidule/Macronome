...
// is i18n (`cibles.targets.presets.<key>`); the values are data and live here.
...
export function RatioPresets({ presets, onPick }: RatioPresetsProps) {
  ...
  return (
    <div className={styles.presets}>
      <span className="hint">{t('cibles.targets.presets.hint')}</span>
      <div className={styles.presetList}>
        {presets.map((p) => (
          <button
            ...
            aria-label={t('cibles.targets.presets.aria', { value: ratio2(p.value) })}
          >
            <span className={styles.presetVal}>{ratio2(p.value)} g/kg</span>
            <span className={styles.presetCaption}>{t(`cibles.targets.presets.${p.key}`)}</span>
          </button>
        ))}
      </div>
    </div>
