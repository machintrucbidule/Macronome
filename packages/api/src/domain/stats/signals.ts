...
/** 30-day avg vs the band, current NOK run (≥ alert), and the 14-day OK rate readout. */
export function signals(logged: DayStat[], zone: TargetZone | null, nokRunAlert: number): Signal[] {
  ...
  if (nokRun >= nokRunAlert) {
    out.push({ code: 'nok_run', value: nokRun, text: `${nokRun} consecutive days off target` });
  }

  const okRate14 = windowStats(logged, anchor, 14).ok_rate;
  if (okRate14 !== null) {
    const value = Math.round(okRate14 * 100);
    out.push({ code: 'ok_rate_14', value, text: `14-day OK rate ${value}%` });
  }
