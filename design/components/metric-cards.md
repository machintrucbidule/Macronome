...
- **Constat** (`.constat`): `--font-num; --fs-10; color:var(--text-faint);
  right-aligned; line-height:1.55`. Deficit/surplus readout; `.def.neg` →
  `--ok`, `.def.pos` → `--nok` (deficit reads negative/green, surplus red).
