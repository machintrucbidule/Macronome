...
- **Auto weight (RW-1)** — `batch_weight_auto` persists the auto-vs-manual state:
  while **auto**, `total_batch_grams ≡ total_ingredient_grams`, refreshed by the
  server on **every** derived-food rebuild (own save **and** parent cascade from a
  nested-recipe edit); while **manual**, the stored value is frozen until edited.
  Setting an explicit cooked weight turns the flag off.
...
note: if the cooked bake is weighed at 900 g, set total_batch_grams=900 (this
  turns batch_weight_auto off) → per100_kcal = 640/900×100 = 71.1 ; per-portion
  macros unchanged (160.0). While batch_weight_auto stays on, editing the
  ingredients (e.g. dropping Ingredient C) re-tracks total_batch_grams to the
  new Σ (500) with no manual action.
```
