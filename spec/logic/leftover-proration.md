...
## 6b. Consumed quantity for display (B-047)

The Qté column shows the **consumed** amount, expressed in the line's own unit, so it stays
consistent with the line's macros and the meal total. Because proration is linear in grams:

```
consumed_quantity_i = served_quantity_i × consumed_grams_i / served_grams_i
```

It equals `served_quantity` when the line is in no group. This is a **display projection** of
the figures above — the proration maths are unchanged. Editing the Qté field still writes the
**served** quantity (the server re-prorates). A draft (pre-apply) deduction is previewed via the
stateless preview endpoint (`spec/api/days-meals-leftover.md` §Leftover), never computed on the
client.
