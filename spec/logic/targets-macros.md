...
## 6. Target BMI (derived; B-042)

The "Objectif de poids" group shows a derived **target BMI** — the BMI the user
would reach at their target weight — using the same formula as the weight-tracking
BMI (`weight-periods-trajectory.md §5`), but on the **target** weight rather than
the current one:

```
target_bmi = target_weight_kg / (height_cm/100)²
```

- `target_bmi` is **null** when `target_weight_kg` is not set (the goal is optional).
  It does not depend on a weigh-in (it uses the goal weight, not the current one).
- Stored exact; the web rounds to 1 decimal for display (`00-conventions.md`).
- Carried by the engine readout (`GET /target`, `POST /target/preview`).
- **Worked example** (oracle):
  `inputs: target_weight_kg=72, height_cm=180`
  `target_bmi = 72 / (1.80)² = 72 / 3.24 = 22.222… → 22.2 (display)`.
